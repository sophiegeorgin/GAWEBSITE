...
AFRAME.registerComponent("looper-rotation-reader", {
  tick: function () {
    ...
    if (
      myStartPosition_x < camera_right_x &&
      myStartPosition_x > camera_left_x &&
      myStartPosition_z < camera_forward_z + 1.5
    ) {
...
function room(camera_forward_z, myPlane, closingDoor) {
  ...
  entityEl.setAttribute(
    "gltf-model",
    "url(galleryimgs/chandeleur/chandeleur.gltf)"
  );
  ...
  entityEl1.setAttribute("gltf-model", "url(galleryimgs/scene.gltf)");
  ...
  entityEl2.setAttribute("gltf-model", "url(galleryimgs/scene.gltf)");
  ...
  for (j = 0; j < 3; j++) {
    ...
    var entityEl = document.createElement("a-entity");
    entityEl.setAttribute("gltf-model", "url(galleryimgs/wallight.gltf)");
    ...
    var entityEl = document.createElement("a-plane");
    painting_no = Math.floor(Math.random() * 6);
    var path = `galleryimgs/paint${painting_no}.jpg`;
    entityEl.setAttribute("material", { src: `url(${path})` });
    entityEl.setAttribute("position", { x: 2.73, y: 2.3, z: myPlane });
    entityEl.setAttribute("rotation", { x: 0, y: -90, z: 0 });
    entityEl.setAttribute("color", "white");
